      ...
      return (
        <div className={styles.fileListItem} key={document.id}>
          <label className={styles.fileTypeLabel}>
            {document.type === documentTypes.DRIVING_LICENSE ? 'Driving License' : 'Other Document'}
          </label>
          <a href={getImageUrl(document.img)} target="_blank" rel="noopener noreferrer" className={styles.imageLabel}>
            View Image
          </a>
        </div>
      );
        ...
        <div className={styles.innerDiv}>
          ...
          <div className={styles.editDiv}>
            ...
            <div className={styles.rightDiv}>
              ...
              {documents.length > 0 && <div className={styles.fileListDiv}>{renderFileList()}</div>}
              <div {...getRootProps({ className: styles.dropzone })}>
                <input {...getInputProps()} />
                {file ? (
                  <div className={styles.displayDiv}>
                    <img className={styles.image} src={filePreview} alt="Selected" />
                    <button className={styles.removeBtn} type="button" onClick={handleRemoveFile}></button>
                  </div>
                ) : (
              ...
              <button type="button" className={styles.uploadBtn} onClick={handleFileUpload} disabled={file === null}>
                UPLOAD
              </button>
            </div>
          </div>
          {documentError && <div className={styles.errorDiv}>{documentError}</div>}
        </div>
      </div>
    );
  };
